...
function sl_enqueue_scripts() {
    wp_enqueue_script('azexo-simple-likes-public-js', get_template_directory_uri() . '/post-like-system/js/simple-likes-public.js', array('jquery'), '0.5', false);
    wp_localize_script('azexo-simple-likes-public-js', 'simpleLikes', array(
        'ajaxurl' => admin_url('admin-ajax.php'),
        'like' => esc_html__('Like', 'medican'),
        'unlike' => esc_html__('Unlike', 'medican')
    ));
}
...
function process_simple_like() {
    ...
    if (!wp_verify_nonce($nonce, 'simple-likes-nonce')) {
        exit(esc_html__('Not permitted', 'medican'));
    }
...
function get_simple_likes_button($post_id, $is_comment = NULL) {
    ...
    if (already_liked($post_id, $is_comment)) {
        ...
        $title = esc_html__('Unlike', 'medican');
        $icon = $icon_full;
    } else {
        $class = '';
        $title = esc_html__('Like', 'medican');
        $icon = $icon_empty;
    }
...
function get_like_count($like_count) {
    $like_text = esc_html__('Like', 'medican');
...
function show_user_likes($user) {
    ...
    <table class="form-table">
        <tr>
            <th><label for="user_likes"><?php _e('You Like:', 'medican'); ?></label></th>
            <td>
                <?php
    ...
    <?php else : ?>
                    <p><?php _e('You do not like anything yet.', 'medican'); ?></p>
    <?php
    endif;
    wp_reset_postdata();
    ?>
