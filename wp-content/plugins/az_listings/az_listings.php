...
function azl_add_custom_field($cmb, $params) {
    if (!isset($params['default'])) {
        $params['default'] = 'azl_get_default';
    }
...
function azl_cmb2_sanitize_text_url_fix($override_value, $value, $object_id, $field_args, $sanitize_object) {
    ...
    $default = '';
    if ($field_args['default'] != 'azl_get_default') {
        //recoursion: get_default -> sanitize_text_url -> get_default
        //$default = $sanitize_object->field->args('default');
    }
    // for repeatable
...
add_filter('wpcf7_form_hidden_fields', 'azl_form_hidden_fields');

function azl_form_hidden_fields($hidden_fields) {
    global $post;
    if (is_single()) {
        $hidden_fields['_post_author'] = $post->post_author;
    }
    if (is_page()) {
...
function azl_before_send_mail($contact_form) {
    $submission = WPCF7_Submission::get_instance($contact_form);
    if (isset($submission->posted_data['_post_author'])) {
        $properties = $contact_form->get_properties();
        $user_info = get_userdata($submission->posted_data['_post_author']);
        $properties['mail']['recipient'] = $user_info->user_email;
        $contact_form->set_properties($properties);
    }
}

add_filter('wpcf7_special_mail_tags', 'azl_special_mail_tags', 10, 3);

function azl_special_mail_tags($output, $name, $html) {
...
function azl_template_redirect() {
    if (isset($_GET['azl']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
        switch ($_GET['azl']) {
            ...
            case 'claim':
                if (is_user_logged_in()) {
                    update_post_meta($_GET['id'], 'claim', get_current_user_id());
                } else {
