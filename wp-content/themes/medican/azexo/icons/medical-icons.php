    ...
    if (isset($settings['params']) && is_array($settings['params'])) {
        ...
        foreach ($params as &$param) {
            if ($param['param_name'] == 'icon_library') {
                $param['value'][esc_html__('Medical icons', 'medican')] = 'medicalicons';
                break;
            }
            $i++;
        }
        array_splice($params, $i + 1, 0, array(array(
                'type' => 'iconpicker',
                'heading' => esc_html__('Icon', 'medican'),
                'param_name' => 'icon_medicalicons',
                'group' => esc_html__('Media', 'medican'),
                ...
                'description' => esc_html__('Select icon from library.', 'medican'),
        )));
        vc_map_update('azexo_generic_content', 'params', $params);
    }

    add_filter("shortcode_atts_azexo_generic_content", 'shortcode_atts_azexo_generic_content', 10, 3);

    function shortcode_atts_azexo_generic_content($out, $pairs, $atts) {
