...
function azqf_radio_render($output, $field) {
    ...
    foreach ($terms as $term) {
        $output .= '<li><input type="radio" id="in-' . $field['taxonomy'] . '-' . $term->term_id . '" name="' . $field['taxonomy'] . '" value="' . $term->slug . '" ' . ($term->slug == $selected ? 'checked' : '') . '><label for="in-' . $field['taxonomy'] . '-' . $term->term_id . '">' . $term->name . '</label></li>';
    }
    $output .= '</ul></div>';
    return $output;
}
