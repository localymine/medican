...
define('AZEXO_FRAMEWORK_VERSION', '1.22');
...
function azexo_admin_scripts() {
    ...
    wp_localize_script('azexo-admin', 'azexo', array(
        ...
        'templates_configuration' => esc_html__('Templates configuration', 'medican'),
        'fields_configuration' => esc_html__('Fields configuration', 'medican'),
        'post_types_settings' => esc_html__('Post types settings', 'medican'),
        'woocommerce_templates_configuration' => esc_html__('WooCommerce templates configuration', 'medican'),
        'section_alert' => esc_html__('Please click "Ok" to confirm this info. Theme not provide CSS styles for all possible configurations in this section. Please use configurations only provided with demo content. Of course you can use it with complete freedom, but in this case it may be necessary to add CSS styles. You can save confirmation of this alert in "AZEXO Options > General settings".', 'medican'),
...
function azexo_get_templates() {
    ...
    $azexo_templates = array_merge($azexo_templates, array(
        ...
        'navigation_post' => esc_html__('Navigation post', 'medican'), //fixed in full post navigation
...
function azexo_admin_bar_menu($wp_admin_bar) {
    $args = array(
        'id' => 'edit-links',
        'title' => esc_html__('Edit links', 'medican'),
        'href' => '#',
        'meta' => array(
            'class' => 'active',
        ),
...
function azexo_scripts() {
    ...
    if (in_array('administrator', (array) $user->roles)) {
        $fontend_variables['edit_links'] = array();
        $fontend_variables['edit_button'] = esc_html__('Edit', 'medican');
        $post_type_object = get_post_type_object('post');
        $fontend_variables['edit_url'] = esc_url(admin_url(sprintf($post_type_object->_edit_link . '&action=edit', 0)));
        if (function_exists('get_redux_instance')) {
            ...
            $fontend_variables['edit_links']['configure_templates'] = array(
                'links' => $configure_templates,
                'text' => esc_html__('Configure template', 'medican'),
                'target' => '_blank',
            );
            wp_enqueue_script('azexo-edit-links', get_template_directory_uri() . '/js/edit-links.js', array('jquery'), AZEXO_FRAMEWORK_VERSION, true);
        }
    }
    wp_enqueue_script('azexo-fontend', get_template_directory_uri() . '/js/fontend.js', array('jquery'), AZEXO_FRAMEWORK_VERSION, true);
    wp_localize_script('azexo-fontend', 'azexo', $fontend_variables);
...
function azexo_styles() {
    ...
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        ...
        foreach ($options['google_font_families'] as $font_family) {
            ...
            if ('off' !== esc_html_x('on', $font[0] . ' font: on or off', 'medican')) {
...
function azexo_widgets_init() {
    if (function_exists('register_sidebar')) {
        ...
        register_sidebar(array('name' => esc_html__('Dashboard sidebar', 'medican'), 'id' => "dashboard_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Additional sidebar', 'medican'), 'id' => "additional_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
    }
}
...
function azexo_before_list($post_type) {
    ...
    if (isset($options[$post_type . '_custom_sorting_numeric_meta_keys']) && is_array($options[$post_type . '_custom_sorting_numeric_meta_keys'])) {
        foreach ($options[$post_type . '_custom_sorting_numeric_meta_keys'] as $meta_key) {
            if (!empty($meta_key)) {
                $orderby_options[esc_attr($meta_key) . '-desc'] = sprintf(esc_attr__('Sort by %s: high to low', 'medican'), esc_attr($meta_key));
                $orderby_options[esc_attr($meta_key) . '-asc'] = sprintf(esc_attr__('Sort by %s: low to high', 'medican'), esc_attr($meta_key));
            }
        }
    }
                ...
                case 'result_count':
                    ?><p class="result-count"><?php
                        ...
                        if ($total <= $per_page || -1 === $per_page) {
                            printf(_n('Showing the single result', 'Showing all %d results', $total, 'medican'), $total);
                        } else {
                            printf(_nx('Showing the single result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, '%1$d = first, %2$d = last, %3$d = total', 'medican'), $first, $last, $total);
                        }
                        ?></p><?php
                            break;
                        case 'ordering':
...
function azexo_entry_field($name) {
    ...
    switch ($name) {
        ...
        case 'post_comments_count':
            ...
            break;
        case 'post_read_more':
            $more_link_text = sprintf(esc_html__('Contact', 'medican'));
...
function azexo_entry_share() {
    ...
    print '<a class="twitter-share" target="_blank" href="https://twitter.com/home?status=' . rawurlencode(esc_attr__('Check out this article: ', 'medican')) . rawurlencode(get_the_title()) . '%20-%20' . rawurlencode(esc_url(apply_filters('the_permalink', get_permalink()))) . '"><span class="share-box"><i class="fa fa-twitter"></i></span></a>';
    if (!empty($image)) {
...
function azexo_get_dashboard_links() {
    ...
    $links = array(
        array(
            ...
            'title' => esc_html__('Sign out', 'medican'),
        )
    );
    $options = get_option(AZEXO_FRAMEWORK);
