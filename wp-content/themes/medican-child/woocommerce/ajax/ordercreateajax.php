            ...
            { 
             $error=1;
               ...
               if(isset($_POST['fromid'])&& ($_POST['fromid']=='orderupload'))
				...
				$myproductID = $searcharray['myproductID'];
				$storedomain = get_post_meta($myproductID,'store_domain',true);
				$storeaddress = get_post_meta($myproductID,'store_address',true);
				$storeprovince = get_post_meta($myproductID,'provineName',true);
				$storedist = get_post_meta($myproductID,'districtName',true);
				$storeward = get_post_meta($myproductID,'wardName',true);
                $productcount = count($productid);
                $Prefix = $_SESSION['myordertype'] = 'B';
                $order = wc_create_order();
				$ordertime =  $order->post->post_date_gmt;
               ...
               $pic_siz = getimagesize($_FILES['file']['tmp_name']);
               $width = $pic_siz[0];
               $height = $pic_siz[1];
...
//               $path = get_stylesheet_directory_uri().'/woocommerce/orderimage/resize_'.$image_path;
               ...
               $obj = new sendMail();
			   $obj->customSendmail($useremail,$userphone,$consumer_id,'Order Success','user_order','',$neworderid,$orderdate,$myproduct,$storedomain,$storeaddress,$storeprovince,$storedist,$storeward,$ordertime);
...
$emailmessage .= '<br>For any query or more information feel free to contact us by email at cskh@webnhathuoc.vn.'; */
               ...
               $error=0; 
				...
				$myproductID = $inputorder_array['myproductID'];
				$storedomain = get_post_meta($myproductID,'store_domain',true);
				$storeaddress = get_post_meta($myproductID,'store_address',true);
				$storeprovince = get_post_meta($myproductID,'provineName',true);
				$storedist = get_post_meta($myproductID,'districtName',true);
               ...
               wp_set_object_terms($post_id, 46, 'product_cat');
			   ...
			   $neworderid = $consumer_id.$order_id;
$obj = new sendMail();
$obj->customSendmail($useremail,$userphone,$consumer_id,'Order Success','user_order','',$neworderid,$delivery_date,$myproduct,$storedomain,$storeaddress,$storeprovince,$storedist,$storeward,$ordertime);
